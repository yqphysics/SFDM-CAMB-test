    ...
    if (.not.this%CP%DarkMatter%calculated_CSF) then
        ...
        if (a< this%CP%DarkMatter%a_arr(2)) then
            rho1 = this%CP%DarkMatter%rho_phi(1)

            grho_phia2 = kappa * rho1 * a**4._dl/c**2

        else

            if (a< this%CP%DarkMatter%a_wkb) then 

                call Irreg%Init(this%CP%DarkMatter%a_arr, this%CP%DarkMatter%rho_phi)
                grho_phia2 = kappa * Irreg%Value(a) * a**4._dl/c**2
                
            else
                grho_phia2 = 3._dl *(1000._dl)**2._dl * 100._dl**2 * this%CP%DarkMatter%omphih2*a/c**2
    ...
    ! write(*,*) 'a = ', a
    ! write(*,*) 'grho_phia2 =', grho_phia2
    ! write(*,*) 'grhov_ta2 = ', grhov_t*a**2 
    ! write(*,*) "dtauda1 =", dtauda
    ...
    !!!!  complex SFDM 绝热初始条件
    initv(1,i_clx_phi) = initv(1,i_clxb)*(1.0_dl + CP%DarkMatter%w_phi(1))
    initv(1,i_q_phi) = 0._dl
    initv(1,i_clx_phi_n) = initv(1,i_clxb)
    initv(1,i_epsilon) = 0._dl
    ...
    use constants, only : barssc0, Compton_CT, line21_const, hbar,c
    use MassiveNu
    use Recombination
    implicit none
    type(EvolutionVars) EV
    ...
    real(dl) grho_phi_t, clx_phi, q_phi, clx_phi_n, epsilon, Delat_P_phi  ! 8*pi*G*rho*a^4, delta, q, delta_n, epsilon, deltaP/deltarho
    real(dl) clx_phidot, q_phidot, clx_phi_ndot , epsilondot
    real(dl) w_phi, cs2_phi, mu_phi
    real(dl), parameter :: eVot = 1.602176634e-19_dl                      ! 电子福特， 由于 eV 和 EV 名称重复， 不能直接导出 
    ...
    mu_phi =  State%CP%DarkMatter%m_phi*eVot/hbar

    
    Delat_P_phi = clx_phi + 3*adotoa*(1-cs2_phi)*(q_phi - epsilon )/k + (1-w_phi)/(1+w_phi) * (4*mu_phi**2*a2)/(3*k*adotoa*c**2*(1-cs2_phi))*epsilon



    write(*,*) '         w_phi        ', '         cs2_phi         ', '          Delat_P_phi          ',  '              clx_phi             ', '        q_phi        ' , '        clx_phi_n        ', '            epsilon              '
    write(*,*)  w_phi ,  cs2_phi ,  Delat_P_phi ,   clx_phi , q_phi ,  clx_phi_n , epsilon

    ! write(*,*) '         w_phi        ', '         cs2_phi         ', '          Delat_P_phi          ', ' term'
    ! write(*,*)  w_phi ,  cs2_phi ,  Delat_P_phi ,  (1-w_phi)/(1+w_phi) * (4*mu_phi**2*a2)/(3*adotoa*(1-cs2_phi))*epsilon




    clx_phidot = 3*adotoa*w_phi*clx_phi - 3*adotoa*Delat_P_phi - k* q_phi - (1+w_phi)*k*z
    q_phidot = - adotoa*(1-3*w_phi)*q_phi + k*Delat_P_phi
    clx_phi_ndot = - 1._dl/(1._dl+w_phi)*k*(q_phi - epsilon ) - k*z
    epsilondot = - adotoa*(1-3*w_phi)*epsilon + k*clx_phi - (1+w_phi)*k*clx_phi_n - (2*w_phi)/(1+w_phi) *(4*mu_phi**2*a2)/(3*adotoa*c**2*(1-cs2_phi)) *epsilon
