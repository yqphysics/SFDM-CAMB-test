...
def set_params(cp=None, verbose=False, **params):
    ...
    * :meth:`.nonlinear.Halofit.set_params`
    ...
    #### 设置初始化暗物质模块
    if cp.DarkMatter is None:
        cp.DarkMatter = DarkMatterCSF()
    ...
    do_set(cp.DarkMatter.set_params)    # 设置 CSF 参数
...
def get_valid_numerical_params(transfer_only=False, **class_names):
    ...
    # extract_params(cp.DarkMatter.set_params)      # 暗物质
...
CSF_Background = camblib.__darkmattercsf_MOD_calcsfbackground
CSF_Background.argtypes = [POINTER(CAMBparams), POINTER(c_bool)]

def csf_background(pars, get_phi = False):
    '''
    计算 CSF 背景演化
    '''
    CSF_Background(byref(pars),c_bool(get_phi))
