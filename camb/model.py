...
from .dark_matter import DarkMatterCSF
...
Transfer_csf_n = 16
Transfer_epsilon = 17
Transfer_max = Transfer_epsilon   ### Transfer_vel_baryon_cdm
...
transfer_names = ['k/h', 'delta_cdm', 'delta_baryon', 'delta_photon', 'delta_neutrino', 'delta_nu', 'delta_tot',
                  ...
                  'delta_CSF', 'v_CSF', 'delta_n_CSF' ]                ##### 添加了 complex SFDM 的相关转移函数名
...
class CAMBparams(F2003Class):
    ...
    _fields_ = [
        ...
        ("DarkMatter", DarkMatterCSF),                # 暗物质模块
    ...
    def set_classes(self, dark_energy_model=None, dark_matter_model=None, initial_power_model=None,
        ...
        :parma dark_matter_model: 'CSF', or name of a DarkMatter class 新加入暗物质
        ...
        # 暗物质
        if dark_matter_model:
            self.DarkMatter = self.make_class_named(dark_matter_model,DarkMatterModel)
    ...
    def set_dark_matter(self, All_DM_is_CDM = True, DM_include_CSF = False,
                     m_phi = 1e-25*eV , Num_density = 1e-9 , omphih2 = 0.0001, dotR = 0.0, dark_matter_model ='CSF'):
        '''
        在 model 中设置 CSF 参数
        '''
        if dark_matter_model == 'CSF':
            dm = self.make_class_named(dark_matter_model, DarkMatterModel)
            dm.set_params(All_DM_is_CDM, DM_include_CSF)
            dmCSF = self.DarkMatter.DarkMatterCSF
            dmCSF.set_CSFparams(m_phi, Num_density, omphih2, dotR)
