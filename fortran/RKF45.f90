module RKF45
  ...
  interface
    subroutine fun_obj( neqn ,t, y, dydt, obj)
      use Precision
      implicit none
      integer, intent(in) :: neqn
      real(dl), intent(in) :: t
      real(dl), intent(in) :: y(neqn)
      real(dl) , intent(out) :: dydt(neqn)
      class(*), optional :: obj
    end subroutine fun_obj
  end interface
...
subroutine oderk45(parmas, fin, neqn, t_start, t_stop, n_step, y0, tval, yval, option, log)
  use iso_c_binding
  implicit none
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !!!  输入量  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  class(*), optional :: parmas
  external :: fin
  procedure(fun_obj), pointer :: fun
  integer, intent(in) :: neqn                          ! 方程数量
  real (dl), intent(in) :: t_start                     ! 初始坐标
  real (dl), intent(in) :: t_stop                      ! 终止坐标
  integer , intent(in) :: n_step                       ! 区间数量
  real (dl), intent(in) :: y0(neqn)                    ! 初始条件
  class(optionD), optional :: option                   ! 精度设置
  ...
  !!!  输出量  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  real (dl), intent(out) :: tval(n_step+1)
  real (dl), intent(out) :: yval(neqn,n_step+1)
  ...
  call C_F_PROCPOINTER(c_funloc(fin),fun)
  ...
  if (present(option)) then
    abserr = option%abserr
    relerr = option%relerr
    flag = option%flag
  else
  ...
  do i_step = 1, n_step
    ...
    if(log_local) then
      t = ( real ( n_step - i_step + 1, dl ) * log10(t_start)  &
          + real (          i_step - 1, dl ) * log10(t_stop) ) & 
          / real ( n_step,              dl )

      t = 10._dl**(t)

      t_out = ( real ( n_step - i_step, dl ) * log10(t_start)  &
              + real (          i_step, dl ) * log10(t_stop) ) & 
              / real ( n_step,          dl )
      ...
      t = ( real ( n_step - i_step + 1, dl ) * t_start  &
          + real (          i_step - 1, dl ) * t_stop ) & 
          / real ( n_step,              dl )
  
      t_out = ( real ( n_step - i_step, dl ) * t_start  &
              + real (          i_step, dl ) * t_stop ) & 
              / real ( n_step,          dl )
      ...
      call r8_rkf45 ( fun, neqn, y, yp, t, t_out, relerr, abserr, flag , parmas )


      do ix = 1 , neqn
        yval(ix,i_step+1) = y(ix)
      enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine r8_fehl ( fin, neqn, y, t, h, yp, f1, f2, f3, f4, f5, s, parmas )
  ...
  !
    ...
    call f ( neqn, t + ch, f5, f1, parmas )
  
    ch = 3.0_dl * h / 32.0_dl
  
    f5(1:neqn) = y(1:neqn) + ch * ( yp(1:neqn) + 3.0_dl * f1(1:neqn) )
  
    call f ( neqn, t + 3.0_dl * h / 8.0_dl, f5, f2, parmas )
  
    ch = h / 2197.0_dl
  
    f5(1:neqn) = y(1:neqn) + ch * &
    ( 1932.0_dl* yp(1:neqn) &
    + ( 7296.0_dl * f2(1:neqn) - 7200.0_dl * f1(1:neqn) ) &
    )
  
    call f ( neqn, t + 12.0_dl * h / 13.0_dl, f5, f3, parmas )
  
    ch = h / 4104.0_dl
  
    f5(1:neqn) = y(1:neqn) + ch * &
    ( &
      ( 8341.0_dl * yp(1:neqn) - 845.0_dl * f3(1:neqn) ) &
    + ( 29440.0_dl * f2(1:neqn) - 32832.0_dl * f1(1:neqn) ) &
    )
  
    call f ( neqn, t + h, f5, f4 , parmas )
    ...
    ( &
      ( -6080.0_dl * yp(1:neqn) &
      + ( 9295.0_dl * f3(1:neqn) - 5643.0_dl * f4(1:neqn) ) &
      ) &
    + ( 41040.0_dl * f1(1:neqn) - 28352.0_dl * f2(1:neqn) ) &
    )
  
    call f ( neqn, t + h / 2.0_dl, f1, f5 , parmas )
  !
  ...
  !
    ...
    ( &
      ( 902880.0_dl * yp(1:neqn) &
      + ( 3855735.0_dl * f3(1:neqn) - 1371249.0_dl * f4(1:neqn) ) ) &
    + ( 3953664.0_dl * f2(1:neqn) + 277020.0_dl * f5(1:neqn) ) &
    )
  ...
  subroutine r8_rkf45 ( fin, neqn, y, yp, t, tout, relerr, abserr, flag , parmas )
  ...
  !
    ...
    integer :: neqn
    real(dl) :: abserr
    real(dl), save :: abserr_save = -1.0_dl
    real(dl) :: ae
    real(dl) :: dt
    real(dl) :: ee
    real(dl) :: eeoet
    ...
    real(dl) :: relerr
    real(dl) :: relerr_min
    real(dl), save :: relerr_save = -1.0_dl
    real(dl), parameter :: remin = 1.0E-12
    real(dl) :: s
    real(dl) :: scale
    real(dl) :: t
    real(dl) :: tol
  ...
  !
    ...
    if ( relerr < 0.0_dl ) then
      flag = 8
      return
    end if
  
    if ( abserr < 0.0_dl ) then
      flag = 8
      return
    end if
  ...
  !
    if ( mflag /= 1 ) then
      ...
      if ( mflag == 2 ) then
        ...
        else if ( kflag == 5 .and. abserr == 0.0_dl ) then
  ...
  !
        ...
        else if ( flag == 5 .and. 0.0_dl < abserr ) then
  ...
  !  accuracy requests.
  !
    relerr_min = 2.0_dl * epsilon ( relerr_min ) + remin
  !
  ...
  !
    ...
    if ( init == 0 ) then
      ...
      h = max ( h, 26.0_dl * eps * max ( abs ( t ), abs ( dt ) ) )
  ...
  !
    if ( abs ( dt ) <= 26.0_dl * eps * abs ( t ) ) then
      t = tout
  ...
  !
        call r8_fehl ( f, neqn, y, t, h, yp, f1, f2, f3, f4, f5, f1, parmas )
        nfe = nfe + 5
  !
  ...
  !
        ...
        do k = 1, neqn
          ...
          ee = abs &
          ( ( -2090.0_dl * yp(k) &
            + ( 21970.0_dl * f3(k) - 15048.0_dl * f4(k) ) &
            ) &
          + ( 22528.0_dl * f2(k) - 27360.0_dl * f5(k) ) &
          )
        ...
        esttol = abs ( h ) * eeoet * scale / 752400.0_dl
  
        if ( esttol <= 1.0_dl ) then
          exit
        end if
  !
  ...
  !
        ...
        if ( esttol < 59049.0_dl ) then
          s = 0.9_dl / esttol**0.2_dl
        else
          s = 0.1_dl
        end if
  ...
  !
      if ( 0.0001889568_dl < esttol ) then
        s = 0.9_dl / esttol**0.2_dl
      else
        s = 5.0_dl
      end if
