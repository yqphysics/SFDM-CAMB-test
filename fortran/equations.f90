    ...
    class(CAMBdata) :: this
    real(dl), intent(in) :: a
    real(dl) :: dtauda, grhoa2, grhov_t, grho_phia2
    real(dl) :: H
    Type(TCubicSpline) :: Irreg



    if (.not.this%CP%DarkMatter%calculated_CSF) then
        ...
        grho_phia2 = this%CP%DarkMatter%grho_phia2(a)

        call this%CP%DarkEnergy%BackgroundDensityAndPressure(this%grhov, a, grhov_t)
        !  8*pi*G*rho*a**4.
        grhoa2 = this%grho_no_de(a) +  grhov_t * a**2 + grho_phia2
        if (grhoa2 <= 0) then
            call GlobalError('Universe stops expanding before today (recollapse not supported)', error_unsupported_params)
            dtauda = 0
        else
    ...
    ! call this%CP%DarkEnergy%BackgroundDensityAndPressure(this%grhov, a, grhov_t)
    ! !  8*pi*G*rho*a**4.
    ! grho_phia2 = 3._dl *(1000._dl)**2._dl * 100._dl**2 * this%CP%DarkMatter%omphih2/c**2*a**4
    ...
    integer, parameter :: basic_num_eqns = 8
    integer, parameter :: ix_etak=1, ix_clxc=2, ix_clxb=3, ix_vb=4 !Scalar array indices for each quantity
    integer, parameter :: ix_clx_phi = 5, ix_q_phi = 6 , ix_clx_phi_n = 7, ix_epsilon = 8         !!! complex SFDM 的演化指标
    ...
    integer, parameter :: i_clx_phi = 13, i_q_phi = 14, i_clx_phi_n = 15, i_epsilon = 16         !!!! complex SFDM 初始条件的指标
    integer, parameter :: i_max =  i_epsilon   ! i_vde
    real(dl) initv(6,1:i_max), initvec(1:i_max)
    real(dl) w_phi, cs2_phi
    ...
    !!!!  complex SFDM 绝热初始条件
    !!!!!!! 插值状态方程
    call State%CP%DarkMatter%State_equation(a, w_phi, cs2_phi)

    initv(1,i_clx_phi) = initv(1,i_clxb)*(1.0_dl + w_phi)
    initv(1,i_q_phi)  = 0
    initv(1,i_clx_phi_n) = initv(1,i_clxb)
    initv(1,i_epsilon) = 0
    ...
    ! complex SFDM 
    y(ix_clx_phi) = initv(1,i_clx_phi) 
    y(ix_q_phi) = initv(1,i_q_phi)
    y(ix_clx_phi_n) = initv(1,i_clx_phi_n)
    y(ix_epsilon) = initv(1,i_epsilon)
    ...
    use constants, only : barssc0, Compton_CT, line21_const, hbar,c, Mpc
    use MassiveNu
    use Recombination
    implicit none
    type(EvolutionVars) EV
    ...
    real(dl) dgrho_de, dgq_de, cs2_de, a_osc, a_wkb

    !!!! complex SFDM 方程变量

    real(dl) grho_phi_t, gpres_phi                            ! 8*pi*G*rho*a^2, 8*pi*G*p*a**2
    real(dl) dotgrho, dotgpres                                ! 8*pi*G*rho'*a^2, 8*pi*G*p'*a**2
    real(dl) clx_phi, q_phi, clx_phi_n, epsilon, dPdrho       ! delta, q, delta_n, epsilon, deltaP/deltarho
    real(dl) clx_phidot, q_phidot, clx_phi_ndot , epsilondot  ! delta', q', delta_n', epsilon'
    real(dl) v_phi                                            ! v_phi  
    real(dl) w_phi, cs2_phi, mnorm, cs2_cm
    real(dl) mtoH, eps_phi                                    ! m/H,  判断精度
    real(dl) H0, omR, omm, h
    ...
    H0 = CP%H0
    h =  H0/100._dl
    omR = CP%omnuh2/h**2 + (State%grhornomass + State%grhog)/State%grhocrit
    omm = (CP%omch2 + CP%DarkMatter%omphih2 + CP%ombh2) /h**2
    ...
    ! complex SFDM 变量
    clx_phi = ay(ix_clx_phi)
    q_phi = ay(ix_q_phi)
    clx_phi_n = ay(ix_clx_phi_n)
    epsilon = ay(ix_epsilon)
    ...
    !!! complex SFDM 背景能量演化
    grho_phi_t = State%CP%DarkMatter%grho_phia2(a)/a2
    ...
    !!! 补充了 complex SFDM 的结果
    dgrho_matter=grhob_t*clxb+grhoc_t*clxc + grho_phi_t*clx_phi  ! Baryon + CDM + complex SFDM
    !  8*pi*a*a*SUM[(rho_i+p_i)*v_i]
    !!! 对于 complex SFDM , dgq = 8*pi*a*a*[ (rho_phi + p_phi)*v_i + epsilion ]
    dgq=grhob_t*vb +  grho_phi_t* q_phi
    ...
    grho_matter=grhonu_t+grhob_t+grhoc_t + grho_phi_t   ! 补充了背景 SFDM 能量 grho_phi_t
    grho = grho_matter+grhor_t+grhog_t+grhov_t

    !!! complex SFDM 的状态方程
    call State%CP%DarkMatter%State_equation(a, w_phi, cs2_phi)

    gpres_phi = State%CP%DarkMatter%gpres_phia2(a)/a**2  ! complex SFDM 的压强

    gpres_noDE = gpres_nu + (grhor_t + grhog_t)/3 + gpres_phi 
    ...
    ! complex SFDM equation of motion
    a_osc = State%CP%DarkMatter%a_osc
    a_wkb = State%CP%DarkMatter%a_wkb

    mnorm =  State%CP%DarkMatter%mnorm  ! 将无量纲化质量

    cs2_cm = k**2/(4*mnorm**2*a2)  ! 有效声速

    dotgrho = State%CP%DarkMatter%dotgrho(a)
    dotgpres = State%CP%DarkMatter%dotgpres(a)
    

    !!!! complex SFDM 精确流体力学方程
    ...
    ! write(*,*) 'a=',a
    ! write(*,*) 'w_phi', w_phi,  'c_s^2=',  cs2_phi

    if (abs(w_phi+1._dl)<eps_phi) then
        w_phi = -1._dl + 2._dl/25._dl* mnorm**2*h**2*10**10._dl/c**2*omR*tau**4 + &
                1/75._dl*mnorm**2*h**3*10**15._dl/c**3*omm*sqrt(omR)*tau**5
    endif

    ! write(*,*) 'w_phi=', w_phi,  'c_s^2=',  cs2_phi
    ! write(*,*) abs((w_phi+1._dl))<eps_phi

    if (a<1.2*a_osc) then 

        !!!! 流体速度
        v_phi = (q_phi/k - epsilon)/(1+w_phi)
        !!!! 本构方程
        if (abs((w_phi-1._dl))<eps_phi) then
            dPdrho = clx_phi
        else
            dPdrho = clx_phi + -3*adotoa*(1+w_phi)*(1-cs2_phi)*v_phi + 4*mnorm**2*a2*(w_phi-1)/(3*adotoa*(1+w_phi)*(1-cs2_phi))*epsilon
            ! if (abs((w_phi+1._dl))<eps_phi) then
            !     dPdrho = clx_phi -3*adotoa*(1+w_phi)*(1-cs2_phi)*v_phi
            ! else
            !     dPdrho = clx_phi -3*adotoa*(1+w_phi)*(1-cs2_phi)*v_phi + 4*mnorm**2*a2*(w_phi-1)/(3*adotoa*(1+w_phi)*(1-cs2_phi))*epsilon
            ! endif
        endif
        !!! 连续性方程
        clx_phidot = 3*adotoa*w_phi*clx_phi - 3*adotoa*dPdrho + k* q_phi - (1+w_phi)*k*z
        !!! Navier-Stokes 方程
        q_phidot = -  adotoa*(1-3*w_phi)*q_phi - k*dPdrho
        !!! 粒子数守恒方程
        clx_phi_ndot = - k*z + k2*v_phi
        !!! 输运扩散方程
        epsilondot = - adotoa*(1-3*w_phi)*epsilon -  clx_phi + (1+w_phi)*clx_phi_n - 2*w_phi/(1+w_phi)*(4*mnorm**2*a2)/(3*adotoa*(1-cs2_phi))*epsilon
    else
        !!!! 流体速度
        v_phi = (q_phi/k - epsilon)
        !!! 连续性方程
        clx_phidot = k* q_phi  - k*z
        !!! Navier-Stokes 方程
        q_phidot = -  adotoa*q_phi - k*cs2_cm*clx_phi
        !!! 粒子数守恒方程
        clx_phi_ndot = - k*z + k2*v_phi
        !!! 输运扩散方程
        epsilondot = - adotoa*epsilon - cs2_cm*(clx_phi - clx_phi_n)
    endif

    ! write(*,*) 'v_s =', v_phi 
    ! write(*,*) 'clx_phi=', clx_phi
    ! write(*,*) 'dPdrho =', dPdrho
    ! write(*,*) 'clx_phi=', clx_phi 
    ! write(*,*) 'q_phi=', q_phi 
    ! write(*,*) 'clx_phi_n=', clx_phi_n 
    ! write(*,*) 'epsilon = ', epsilon 

    ayprime(ix_clx_phi) = clx_phidot
    ayprime(ix_q_phi) = q_phidot
    ayprime(ix_clx_phi_n) = clx_phi_ndot
    ayprime(ix_epsilon) = epsilondot
    ...
    if (associated(EV%OutputTransfer) .or. associated(EV%OutputSources)) then
        ...
        if (associated(EV%OutputTransfer)) then
            ...
            !!! complex SFDM 转移函数
            EV%OutputTransfer(Transfer_csf) = clx_phi
            EV%OutputTransfer(Transfer_vel_csf) = (q_phi - epsilon)/(1+w_phi)
            EV%OutputTransfer(Transfer_csf_n) = clx_phi_n
            EV%OutputTransfer(Transfer_epsilon) = epsilon
            if (State%CP%do21cm) then
    ...
    !!!! complex SFDM 
    real(dl) grho_phi_t, gpres_phi, w_phi, cs2_phi
    ...
    ! complex SFDM 状态方程
    call State%CP%DarkMatter%State_equation(a, w_phi, cs2_phi)
    gpres_phi = grho_phi_t*w_phi

    grho=grhob_t+grhoc_t+grhor_t+grhog_t+grhov_t + grho_phi_t 
    gpres=(grhog_t+grhor_t)/3._dl+grhov_t*w_dark_energy_t + gpres_phi
